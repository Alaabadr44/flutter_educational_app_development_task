...
class DropdownFormField<T, RETURNTYPE> extends FormBuilderFieldDecoration<RETURNTYPE> {
  ...
  final RETURNTYPE Function(T e) converter;
  final bool Function(T e, RETURNTYPE value) compare;
  ...
  DropdownFormField({
    ...
    this.decorationField,
          ...
          initialValue: initialValue != null ? converter(initialValue) : null,
          builder: (FormFieldState<RETURNTYPE> field) {
            final state = field as _FormBuilderDropdownState<T, RETURNTYPE>;
            ...
            return DropdownButtonHideUnderline(
              child: DropdownButtonFormField<T>(
                ...
                decoration: decorationField != null
                    ? decorationField.decorationField(state.context)
                    : DecorationField().decorationField(state.context),
                items: items
                    ...
                    .toList(),
                value: (hasValue ? items.firstWhere((element) => compare(element, field.value as RETURNTYPE)) : null),
                style: style,
                isDense: isDense,
                elevation: elevation,
                iconSize: iconSize,
                icon: icon ??
                    const Padding(
                      ...
                      child: ImageWidget(
                        image: Icons.arrow_forward_ios_rounded,
                        width: 20,
                        height: 20,
                      ),
                    ),
                ...
                onChanged: state.enabled ? (T? value) => value != null ? state.didChange(converter(value)) : null : null,
                onTap: onTap,
  ...
  @override
  FormBuilderFieldDecorationState<DropdownFormField<T, RETURNTYPE>, RETURNTYPE> createState() =>
      _FormBuilderDropdownState<T, RETURNTYPE>();
}

class _FormBuilderDropdownState<T, RETURNTYPE>
    extends FormBuilderFieldDecorationState<DropdownFormField<T, RETURNTYPE>, RETURNTYPE> {
  @override
  void didUpdateWidget(covariant DropdownFormField<T, RETURNTYPE> oldWidget) {
    super.didUpdateWidget(oldWidget);

    final oldValues = oldWidget.items.map((e) => e).toList();
    final currentlyValues = widget.items.map((e) => e).toList();
    final oldChilds = oldWidget.items.map((e) => e.toString()).toList();
    final currentlyChilds = widget.items.map((e) => e.toString()).toList();

    bool isContainsCurrentValues = false;

    for (var element in currentlyValues) {
      if (widget.converter(element) == initialValue) {
        isContainsCurrentValues = true;
        break;
      }
    }

    if (!isContainsCurrentValues && !initialValue.emptyValidator()) {
      assert(
        !isContainsCurrentValues && initialValue.emptyValidator(),
