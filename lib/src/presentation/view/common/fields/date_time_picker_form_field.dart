...
class DateTimePickerFormField extends FormBuilderFieldDecoration<DateTime> {
  ...
  @Deprecated('This property is no used anymore. Please use decoration.suffixIcon to set your desired icon')
  ...
  const DateTimePickerFormField({
    ...
    this.onTap,
    this.showHint = false,
  }) : super(initialValue: initialDate, builder: _buildField);

  static Widget _buildField(FormFieldState<DateTime?> field) {
    final state = field as _FormBuilderDateTimePickerState;
    return state.buildForm();
  }

  @override
  FormBuilderFieldDecorationState<DateTimePickerFormField, DateTime> createState() => _FormBuilderDateTimePickerState();
}

class _FormBuilderDateTimePickerState extends FormBuilderFieldDecorationState<DateTimePickerFormField, DateTime> {
  ...
  Widget buildForm() {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          if (widget.showHint && decorationField.hintText!.isNotNull) ...[
            TextWidget(
              text: decorationField.hintText!,
              style: decorationField.hintStyle?.copyWith(color: AppColors.reverseBaseColor),
            ),
            12.ph,
          ],
          InputDecorator(
            decoration: FormFieldBorders.border(
                ...
                decorationField: decorationField.copyWith(labelLock: FloatingLabelBehavior.never),
                context: context),
            child: GestureDetector(
              onTap: () {
                widget.enabled ? onShowPicker(context, value) : null;
                widget.onTap?.call(value);
              },
              child: TextWidget(
                text: value != null ? _dateFormat.format(value!) : widget.decorationField?.hintText ?? '',
  ...
  DateFormat _getDefaultDateTimeFormat() {
    final languageCode = SettingService.getLocale?.languageCode;
    switch (widget.inputType) {
      case InputType.time:
        return DateFormat.Hm(languageCode);
      case InputType.date:
        return DateFormat.yMMMMd(languageCode);
      case InputType.both:
      default:
  ...
  Future<DateTime?> onShowPicker(BuildContext context, DateTime? currentValue) async {
    ...
    switch (widget.inputType) {
      case InputType.date:
        newValue = convertDateOnly(await _showDatePicker(context, currentValue));
        break;
      case InputType.time:
  ...
  Future<DateTime?> _showDatePicker(BuildContext context, DateTime? currentValue) {
    ...
    return showDatePicker(
      ...
      lastDate: widget.lastDate ?? DateTime(now.year + 2000, now.month, now.day),
  ...
  Future<TimeOfDay?> _showTimePicker(BuildContext context, DateTime? currentValue) async {
    ...
    // if (SettingService.getLocale != null) {
  ...
  DateTime? convertDateOnly(DateTime? date) => date == null ? null : DateTime(date.year, date.month, date.day);
