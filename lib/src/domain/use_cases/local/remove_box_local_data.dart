...
class RemoveBoxLocalData<MODEL> extends UseCase<Future<DataState<MODEL>>, LocalParams> {
  final LocalAppRepository _localRepository;

  RemoveBoxLocalData(this._localRepository);

  @override
  Future<DataState<MODEL>> call({required LocalParams event}) async {
    try {
      final result = await _localRepository.removeBox(event);
